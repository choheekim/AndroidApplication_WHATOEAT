...
public class CardContainer extends AdapterView<ListAdapter> {
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                Restaurant cardModel = (Restaurant) getAdapter().getItem(getChildCount()-1);
    ...
    public void leave(float velocityX, float velocityY) {
        if (!locked) {
            ...
            Log.d("findBug", Integer.toString(mNextAdapterPosition));
            Log.d("findBug", Integer.toString(mActivePointerId));
            Log.d("findBug", Integer.toString(getChildCount()));
